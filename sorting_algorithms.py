# Laboratory Work nr. 2 - Sorting Algorithms (QuickSort, MergeSort, HeapSort, SelectionSort)
# Student: Oberșt Eduard
# Group: FAF-243
...
if __name__ == '__main__':
    ...
    for (name, array) in arrays_short.items():
        ...
        start_time = time.perf_counter()
        selection_sort(new_array, len(new_array))
        elapsed_time = time.perf_counter()
        elapsed_time = elapsed_time - start_time
    ...
    print_single_algorithm("SelectionSort", time_values_short)
    print(pretty_table)

    values = time_values_short.values()
    plot_graph("SelectionSort", small_sizes, values)

    all_values_short["SelectionSort"] = list(time_values_short.values())
    ...
    for sort in ["QuickSort", "HeapSort", "MergeSort", "SelectionSort"]:
